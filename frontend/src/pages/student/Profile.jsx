...
import { Dialog, DialogTrigger, DialogDescription, DialogTitle, DialogFooter, DialogContent, DialogHeader } from "@/components/ui/dialog";
...
const Profile = () => {
    ...
    const updateUserHandler = async () => {
        if (!name && !profilePhoto) {
            return toast.error("Please update name or profile photo");
        }
    ...
    return (
        <div className="max-w-4xl mx-auto px-4 my-20">
            ...
            <div className="flex flex-col md:flex-row items-center md:items-start gap-8 my-5">
                ...
                <div>
                    <div className="mb-2">
                        <h1 className="font-semibold text-gray-900 dark:text-gray-100">{user?.name}</h1>
                    </div>
                    <div className="mb-2">
                        <h1 className="font-semibold text-gray-900 dark:text-gray-100">{user?.email}</h1>
                    </div>
                    <div className="mb-2">
                        <h1 className="font-semibold text-gray-900 dark:text-gray-100">
                            Role :
                            <span className="font-bold text-gray-700 dark:text-gray-300 ml-1 uppercase">{user?.role}</span>
                        </h1>
                    </div>
                    <Dialog>
            ...
            <div>
                ...
                <div className="grid grid-cols-1 sm:frid-cols-2 md:grid-cols-3 gap-4 my-5">
                    {
                        user?.enrolledCourses.length === 0 ? <h1>You have'nt enrolled yet</h1> : (
